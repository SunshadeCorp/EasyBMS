 ...
 */
void LTC68042::initSPI(byte pinMOSI, byte pinMISO, byte pinCLK) {
 ...
 */
void LTC68042::wakeup_idle() const {
...
*********************************************************************************************************/
constexpr std::uint16_t LTC68042::calcPEC15(const std::uint16_t data) const {
...
constexpr std::uint16_t LTC68042::calcPEC15(const std::array<std::uint8_t, N> &data) const {
    std::uint16_t remainder = 16, addr = 0;  // initialize the PEC
...
*********************************************************************************************************/
void LTC68042::cfgSetVUV(const float Undervoltage) {
...
float LTC68042::cfgGetVUV() const {
    unsigned int value;
...
the config has to be written to the chip after this!
*********************************************************************************************************/
void LTC68042::cfgSetVOV(const float Overvoltage) {
...
float LTC68042::cfgGetVOV() const {
    unsigned int value;
...
*********************************************************************************************************/
void LTC68042::cfgSetDCC(std::bitset<12> dcc) {
    // assert 0x0fff
...
std::bitset<12> LTC68042::cfgGetDCC() const {
    return std::bitset<12>{regs.CFGR[CFGR4] | (static_cast<unsigned long long>(regs.CFGR[CFGR5] & CFG5_DCC_MSK) << 8)};
}

void LTC68042::cfgSetDischargeTimeout(DischargeTimeout timeout) {
 ...
 */
void LTC68042::cfgSetADCMode(ADCFilterMode mode) {
    switch (mode) {
...
void LTC68042::cfgSetRefOn(const bool value) {
    regs.CFGR0w = (regs.CFGR0w & (~CFG0_REFON_MSK)) | (value << CFGR0_REFON_Pos);
}

bool LTC68042::cfgGetRefOn() {
    return (regs.CFGR0r & CFG0_REFON_MSK);
}

bool LTC68042::cfgGetSWTENPin() const {
...
*********************************************************************************************************/
void LTC68042::cfgWrite()  // A two dimensional array of the configuration data that will be written
{
...
TODO: refactor
*********************************************************************************************************/
float LTC68042::cellComputeSOC(float voc) {
...
*********************************************************************************************************/
void LTC68042::clrAuxRegs() const {
    // 3
    wakeup_idle();  // This will guarantee that the LTC6804 isoSPI port is awake.This command can be removed.
    // 4
...
*********************************************************************************************************/
template <std::size_t N>
bool LTC68042::getCellVoltages(std::array<float, N> &voltages, const CellChannel ch) {
    ...
    switch (ch) {
        ...
        case CellChannel::CH_CELL_3_AND_9:
            if (!spi_read_cmd(RDCVA | am | address, regs.CVAR)) return false;

            if (!spi_read_cmd(RDCVC | am | address, regs.CVCR)) return false;
        ...
        case CellChannel::CH_CELL_6_AND_12:
            if (!spi_read_cmd(RDCVB | am | address, regs.CVBR)) return false;

            if (!spi_read_cmd(RDCVD | am | address, regs.CVDR)) return false;
 ...
 */
template <std::size_t N>
constexpr inline void LTC68042::parseVoltages(const unsigned int group, const std::array<std::uint8_t, SIZEREG> &regGroup, std::array<float, N> &data) {
...
constexpr inline float LTC68042::parseVoltage(const std::array<std::uint8_t, SIZEREG> &regGroup, const RegNames index) {
...
*********************************************************************************************************/
void LTC68042::clrCellRegs() const {
    // 3
    // wakeup_idle (); //This will guarantee that the LTC6804 isoSPI port is awake. This command can be removed.
...
*********************************************************************************************************/
bool LTC68042::checkSPI(const bool dbgOut) {
...
*********************************************************************************************************/
float LTC68042::getAuxVoltage(const AuxChannel chg) {
    wakeup_idle();  // This will guarantee that the LTC6804 isoSPI port is awake, this command can be removed.

    switch (chg) {
        case AuxChannel::CHG_GPIO1:
            if (!spi_read_cmd(RDAUXA | am | address, regs.AVAR)) return NAN;
        ...
        case AuxChannel::CHG_GPIO4:
            if (!spi_read_cmd(RDAUXB | am | address, regs.AVBR)) return NAN;
...
/*!*******************************************************************************************************
 ...
 7. Return Result -1= Error , 0= DataOkay
*********************************************************************************************************/
float LTC68042::getStatusVoltage(const StatusGroup chst) {
    wakeup_idle();  // This will guarantee that the LTC6804 isoSPI port is awake, this command can be removed.

    switch (chst) {
        case StatusGroup::CHST_SOC:
            if (!spi_read_cmd(RDSTATA | am | address, regs.STAR)) return NAN;
        ...
        case StatusGroup::CHST_VD:
            if (!spi_read_cmd(RDSTATB | am | address, regs.STBR)) return NAN;
...
bool LTC68042::getStatusMUXFail() {
    if (!spi_read_cmd(RDSTATB | am | address, regs.STBR)) return false;

    return (regs.STBR[STBR5] & STBR5_MUXFAIL_MSK);
}

bool LTC68042::getStatusThermalShutdown() {
    if (!spi_read_cmd(RDSTATB | am | address, regs.STBR)) return false;
...
std::bitset<12> LTC68042::getStatusOverVoltageFlags() {
    std::bitset<12> ret;
...
std::bitset<12> LTC68042::getStatusUnderVoltageFlags() {
    std::bitset<12> ret;
...
int LTC68042::getStatusRevision() {
...
*********************************************************************************************************/
void LTC68042::startCellConv(DischargeCtrl dcp, CellChannel ch) const {
...
*********************************************************************************************************/
void LTC68042::startCellConvTest(SelfTestMode st) const {
...
*********************************************************************************************************/
void LTC68042::startAuxConv(AuxChannel chg) const {
...
/*!*******************************************************************************************************
  ...
  3. send broadcast adax command to LTC6804
*********************************************************************************************************/
void LTC68042::startCellAuxConv(DischargeCtrl dcp) const {
...
/*!*******************************************************************************************************
  ...
  3. send broadcast adax command to LTC6804
*********************************************************************************************************/
void LTC68042::startStatusConv(StatusGroup chst) const {
...
/*!*******************************************************************************************************
  ...
  3. send broadcast adax command to LTC6804
*********************************************************************************************************/
void LTC68042::startOpenWireCheck(PUPCtrl pup, DischargeCtrl dcp, CellChannel ch) const {
...
void LTC68042::readStatusDbg() {
    ...
    Serial.print(getStatusVoltage(LTC68042::CHST_SOC));
    Serial.println(" V");

    Serial.print("Analog Supply Voltage: ");
    Serial.print(getStatusVoltage(LTC68042::CHST_VA));
    Serial.println(" V");

    Serial.print("Digital Supply Voltage: ");
    Serial.print(getStatusVoltage(LTC68042::CHST_VD));
    Serial.println(" V");
...
*********************************************************************************************************/
void LTC68042::readCellsDbg()  // Array of the parsed cell codes
{
